...
pergunta3P(Sub) :- format('3) O docente é substituto? [s/n]'),
    get_char(Sub), get_char('\n').

respondeP(L,S,Sub) :- docente(L,S,Sub,N), !, format('SISTEMA: Docente encontrado!~nNome: ~w ~n.', [N]).
respondeP(L,S,Sub) :- format('SISTEMA: Docente não encontrado!~nDeseja gravar a resposta na base de dados do sistema? [s/n]~nR: '),
    get_char(R), get_char('\n'), R=s, !,
    gets(N),
